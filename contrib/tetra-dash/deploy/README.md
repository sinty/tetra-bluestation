...
юниты, ни `enable`/`disable` не проходят — и выключение машины кнопкой
«Выключить питание» (только выключение: перезагрузка и сон не разрешены).
Имя пользователя в правиле прописано явно, поменяйте под своё.

Включить машину после такого выключения можно только физически. Флаг
«станция нужна» кнопка не трогает: после подачи питания станция поднимется
в том же состоянии, в каком была.
...
Зависшую машину поднимает только сброс. Чтобы это происходило само:

```sh
sudo install -m 0644 systemd/10-watchdog.conf /etc/systemd/system.conf.d/
sudo systemctl daemon-reexec
```

И на стороне гипервизора — включить само устройство, иначе настройка гостя
ничего не делает. Для Proxmox:

```sh
qm set <VMID> -watchdog model=i6300esb,action=reset
```

После этого машину надо **выключить и включить**, а не сбросить: устройство
добавляется при создании процесса qemu.
